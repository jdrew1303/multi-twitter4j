...
public class OAuthSetupTool {
    ...
    public boolean run(String[] args) {
        ...
        try {
            authorize(consumerKey, consumerSecret);
        } catch (Exception e) {
            e.printStackTrace();
            return false;
        }
    ...
    public void authorize(String consumerKey, String consumerSecret)
        ...
        String str = String.format("bot.%s.oauth.consumerKey=%s\n"
                + "bot.%s.oauth.consumerSecret=%s\n"
                + "bot.%s.oauth.accessToken=%s\n"
                + "bot.%s.oauth.accessTokenSecret=%s\n", screenName,
                consumerKey, screenName, consumerSecret, screenName,
                accessToken.getToken(), screenName,
